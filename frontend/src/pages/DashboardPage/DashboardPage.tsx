...
export default function DashboardPage() {
    ...
    //Redicerionar se não houver arquivo válido
    useEffect(() => {
    ...
    useEffect(() => {
        const loadData = async () => {
            ...
            try {
                ...
                const uniqueFiles = [...new Set(allData.map((item: PatientRecord) => item.source_file).filter((file): file is string => file !== undefined && file !== null))];
                setUploadedFiles(uniqueFiles);

                //3. Filtrar dados pelo arquivo selecionado (se houver)
                let filteredData = allData;
                if (state.selectedFile) {
                    filteredData = allData.filter((item: PatientRecord) => item.source_file === state.selectedFile);
                ...
                const errorMsg = err instanceof Error ? err.message : "Erro ao carregar os dados";

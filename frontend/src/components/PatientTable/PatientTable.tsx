...
export default function PatientTable({ refresh }: PatientTableProps) {
    ...
    const pacientes = [...new Set(data
        .map((d) => d.patient_name)
        .filter(Boolean) // Remove valores vazios/nulos
        .filter((name): name is string => name !== undefined && name !== null) // Garante que é string
    )];
    ...
    return (
        <>
            ...
            <div style={{ 
                ...
                • Pacientes únicos: {pacientes.length}<br/>
                • Exibindo: {dadosOrdenados.length} registros<br/>
                • Paciente selecionado: {pacienteSelecionado || "Todos"}
            </div>
        </>
    );
}